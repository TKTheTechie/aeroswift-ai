<script>
  ...
  const steps = [
    ...
    },
    {
      ...
      actions: [
        { label: 'Camera Feed', handler: () => onEnter?.() },
          ...
          <div class="flex flex-col gap-1.5">
            {#each step.actions as action}
              <button
                ...
                class="inline-flex items-center justify-between gap-1.5 text-aero-teal text-xs font-semibold bg-aero-teal/10 hover:bg-aero-teal/20 border border-aero-teal/20 hover:border-aero-teal/50 rounded-lg px-2.5 py-1.5 transition-all duration-200 group/btn"
              >
                {action.label}
